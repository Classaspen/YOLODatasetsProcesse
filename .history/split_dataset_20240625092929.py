...
def split_label(dataset_all_path, dataset_split_path, train_percent=0.85, val_percent=0.15, test_mode=1):
    ...
    for i in list_all_txt:
        ...
        if i in train:
            ...
            file_train.write(str('images/train/' + name + '.png' + '\n'))
        elif i in val:
            dst_val_Image = val_image_path + name + '.png'
            dst_val_Label = val_label_path + name + '.txt'
            shutil.copyfile(srcImage, dst_val_Image)
            shutil.copyfile(srcLabel, dst_val_Label)
            file_val.write(str('images/val/' + name + '.png' + '\n'))
        else:
            dst_test_Image = test_image_path + name + '.png'
            dst_test_Label = test_label_path + name + '.txt'
            shutil.copyfile(srcImage, dst_test_Image)
            shutil.copyfile(srcLabel, dst_test_Label)
            file_test.write(str('images/test/' + name + '.png' + '\n'))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--split_dir', type=str, default="C:/Users/username/Desktop/HERO/cam0_sentry_B/label_txt/", help='Output location of the split dataset')
